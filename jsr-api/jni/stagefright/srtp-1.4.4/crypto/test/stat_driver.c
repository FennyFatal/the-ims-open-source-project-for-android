...
err_status_t random_source_alloc(void);

#define err_check(s) \
{ \
    err_status_t status = s; \
    if (status) { \
        printf("error (code %d)\n", status); \
        return(status); \
    } \
...
int stat_driver(void) {
    uint8_t buffer[2500];
    unsigned int buf_len = 2500;
    int i, j;
    extern cipher_type_t aes_icm;
    cipher_t *c;
    uint8_t key[30] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x00,
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x00, 0x01, 0x02, 0x03,
