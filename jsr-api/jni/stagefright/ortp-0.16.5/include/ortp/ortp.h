...
// On Android platform, use Android's logging utilities
#if !defined(IS_ANDROID)
...
#else // #if !defined(IS_ANDROID)

#ifdef ORTP_DEBUG_MODE
#define ortp_debug(...) LOGD(__VA_ARGS__)
#else
#define ortp_debug(...)
#endif
...
#define ortp_message(...) LOGI(__VA_ARGS__)
#define ortp_warning(...) LOGW(__VA_ARGS__)

#endif

#define ortp_error(...) LOGE(__VA_ARGS__)
#define ortp_fatal(...) LOGE(__VA_ARGS__)
