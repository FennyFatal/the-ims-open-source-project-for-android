...
unsigned int rtp_sendto(rtp_sender_t sender, const void* msg, int len) {
    int octets_sent;
    err_status_t stat;
    ...
    /* apply srtp */
    stat = asrtpa_protect(sender->asrtpa_ctx, &sender->message.header, &pkt_len);
    if (stat) {
#if PRINT_DEBUG
...
#if VERBOSE_DEBUG
    asrtpa_print_packet(&sender->message.header, pkt_len);
#endif
    octets_sent = sendto(sender->socket, (void*) &sender->message, pkt_len, 0,
...
#endif
#if VERBOSE_DEBUG
    asrtpa_print_packet(&receiver->message.header, octets_recvd);
#endif

    /* apply srtp */
    stat = asrtpa_unprotect(receiver->asrtpa_ctx, &receiver->message.header,
            &octets_recvd);
    if (stat) {
...
int rtp_sender_init(rtp_sender_t sender, int socket, struct sockaddr_in addr,
        unsigned int ssrc) {
...
int rtp_receiver_init(rtp_receiver_t rcvr, int socket, struct sockaddr_in addr,
        unsigned int ssrc) {
...
int rtp_sender_init_srtp(rtp_sender_t sender, const asrtpa_policy_t *policy) {
    return asrtpa_create(&sender->asrtpa_ctx, policy);
}

int rtp_receiver_init_srtp(rtp_receiver_t sender, const asrtpa_policy_t *policy) {
    return asrtpa_create(&sender->asrtpa_ctx, policy);
}

rtp_sender_t rtp_sender_alloc() {
    return (rtp_sender_t) malloc(sizeof(rtp_sender_ctx_t));
}

rtp_receiver_t rtp_receiver_alloc() {
